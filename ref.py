...
def window_slice(data,window_len,overlap_len):
    ...
    segments = aquisition[channel_list,0:window_len]

    segments = segments.view(1,channels_num,window_len)
    ...
    while time + window_len <= total_length:
        segment_inc = aquisition[channel_list,time:time + window_len]
        segment_inc1 = segment_inc.view(1,channels_num,window_len)
...
def load_dataset(sub,ges,trail):
    for i in ges:
        for j in sub:
            for k in trail:
                ...
                if trail.index(k) == 0:
                    trail_total = window_slice(data['data'],WINDOW_LEN,OVERLAP_LEN)
                else:
                    trail_single = window_slice(data['data'],WINDOW_LEN,OVERLAP_LEN)
...
class EMGData(Dataset): #继承Dataset
    def __init__(self,sub,ges,trail): #__init__是初始化该类的一些基础参数
...
if sys.argv[2] == '0':
    ...
    for epoch in range(num_epochs):
        ...
        print ('Epoch [{}/{}], Step [{}/{}], Loss: {:.7f},Time:{}' .format(epoch+1, num_epochs, i+1, total_step, loss.item(),datetime.datetime.now()))
